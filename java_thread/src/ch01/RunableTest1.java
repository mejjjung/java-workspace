...
// 생성 방법, 사용방법을 이미 학습
// 만약 코드가 JFrame을 상속 받았다면 자는 단일 상속을 지원하기 때문에 다중 상속을 할 수 없다.
...
public class RunableTest1 {
	...
	public static void main(String[] args) {
		...
		// subWorker 기능을 구현했지 Thread를 상속 받은 것은 아니다. 그래서
		// thread 기능인 start 메소드가 없는 상태이다.

		// !!!
		// 생각하는 방법 -- start() 메서드는 thread 가지고 있다.
		// 3) 쓰레드를 생성할 때 생성자에 runnable 타입을 넣을 수 있다고 확인 - 문서, 코드 힌트
...
class SubWorker extends JFrame implements Runnable {
	...
	// 1) 나중에 쓰레드가 동작 해야 하는 부분을 run 안에서 작성 함
	@Override
	public void run() {
		for (int i = 0; i < 200; i++) {
